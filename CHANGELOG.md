...
2025-12-09 — Subscription Screen Overhaul + Cache Fixes
🔧 Frontend changes

Полностью переработан экран подписки в Telegram MiniApp.

Обновлены тексты тарифов “Месяц” и “Год”.

Карточки подписки расширены, приведены к единому стилю.

Удалены белые рамки и добавлены мягкие скругления.

Установлены современные кнопки CTA (стиль как у Кинопоиска).

Цена и описание теперь корректно центрируются.

Снижены вертикальные отступы между карточками.

Добавлено отображение имени пользователя рядом с аватаркой.

Убрана техническая обводка вокруг аватара.

🛠 Navigation

Исправлен переход на вкладку “Подписка” в нижнем меню.

⚙️ Caching & infrastructure

Полностью решена проблема агрессивного кеширования Telegram WebView.

Введён механизм cache-busting для HTML/CSS/JS.

Добавлены строгие заголовки Cache-Control для статических файлов.

Исправлены маршруты /api в vercel.json (теперь корректно обрабатываются Vercel Functions).

Устранена ошибка деплоя на Vercel, приложение стабильно собирается.

🧼 Cleanup

Удалено старое кешированное поведение.

Удалены устаревшие части стилей и разметки.